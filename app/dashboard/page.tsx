...
const TICK_STYLE    = { fontSize: 10, fill: "#8892a4" };
const TOOLTIP_STYLE = {
  background:   "#161d2e",
...
export default function DashboardPage() {
  ...
  const pieData = [
    { name: "FAKE", value: stats?.fake ?? 1 },
    { name: "REAL", value: stats?.real ?? 1 },
  ];
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10 space-y-8">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">Model performance · Live prediction stats</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Analyzed", val: stats?.total ?? 0, color: "#00d4aa" },
          { label: "Fake Detected",  val: stats?.fake  ?? 0, color: "#ef4444" },
          { label: "Real Detected",  val: stats?.real  ?? 0, color: "#10b981" },
        ].map(({ label, val, color }) => (
        ...
        <div className="glass rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={EPOCH_DATA}>
              ...
              <YAxis domain={[40, 100]} tick={TICK_STYLE} tickFormatter={(v: number) => `${v}%`} />
              <Tooltip contentStyle={TOOLTIP_STYLE} />
        ...
        <div className="glass rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={MODEL_DATA} layout="vertical">
              ...
              <XAxis type="number" domain={[0.8, 1]} tick={TICK_STYLE} tickFormatter={(v: number) => v.toFixed(2)} />
        ...
        <div className="glass rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-3 mt-2">
            {CONF_MATRIX.map(({ label, val, color }) => (
              <div key={label} className="flex flex-col items-center justify-center rounded-xl border border-white/5 bg-white/3 p-5">
